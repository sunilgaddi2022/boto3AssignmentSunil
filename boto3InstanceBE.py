...
ec2 = boto3.resource('ec2', region_name='ap-east-1')  # You can set your desired region

# AMI ID for Ubuntu 20.04 LTS in ap-east-1 (this may change; always verify the current ID)
ubuntu_ami_id = 'ami-east0287a05f0ef0e6e7f'  # Replace with the actual AMI ID you find for your Ubuntu version and region

# User data script to update packages and install Nginx
password = 'trhyjuyuidwfw'
user_data_script = f"""#!/bin/bash
cd /home/ubuntu/
sudo apt-get update -y
sudo apt-get install git -y
sudo git clone https://github.com/sunilgaddi2022/TravelMemory.git
sudo apt-get update
...
sudo npm install
echo 'MONGO_URI="mongodb+srv://SunilGaddi12:{password}@cluster0.0nvbmll.mongodb.net//travelmemory"' >> .env
echo 'PORT=80' >> .env
sudo apt-get update -y
...
instances = ec2.create_instances(
    ...
    SecurityGroupIds=['sg-072870334bab90a11'],  # Ensure the security group exists and allows appropriate traffic
...
ec2.meta.client.associate_address(InstanceId=instance.id, PublicIp='3.234.220.218')

print("New Ubuntu Backend instance is created and associated with Elastic IP (3.234.220.218).")
